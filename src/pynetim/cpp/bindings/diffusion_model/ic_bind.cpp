...
#include "diffusion_model/independent_cascade.h"

namespace py = pybind11;

PYBIND11_MODULE(independent_cascade_model, m) {
    m.doc() = "独立级联模型（IC），用于社交网络影响力传播模拟";

    {
        ...
        py::class_<pynetim::IndependentCascadeModel>(m, "IndependentCascadeModel")
            .def(py::init([](py::object graph_obj, const std::set<int>& seeds, bool record_activated, bool record_activation_frequency) {
                try {
                    auto graph_ptr = py::cast<std::shared_ptr<pynetim::Graph>>(graph_obj);
                    return std::make_unique<pynetim::IndependentCascadeModel>(graph_ptr, seeds, graph_obj, record_activated, record_activation_frequency);
                } catch (const py::cast_error&) {
                    throw py::type_error("IndependentCascadeModel() 参数错误: graph 必须是 IMGraph 类型。\n用法: IndependentCascadeModel(graph, seeds, record_activated=False, record_activation_frequency=False)");
                }
            }),
                py::arg("graph"), py::arg("seeds"), py::arg("record_activated") = false, py::arg("record_activation_frequency") = false,
                py::keep_alive<0, 1>(),
                R"doc(__init__(graph: IMGraph, seeds: set[int], record_activated: bool = False, record_activation_frequency: bool = False) -> None
...
Args:
    record: 是否记录激活节点。
            ...
            .def("set_record_activation_frequency", &pynetim::IndependentCascadeModel::set_record_activation_frequency,
                py::arg("record"),
...
Args:
    record: 是否记录激活频数。
            ...
            .def("run_single_simulation",
                [](pynetim::IndependentCascadeModel& self, py::object random_seed_obj) {
...
Returns:
    int: 本次模拟激活的节点数。
            ...
            .def("get_activated_nodes",
                &pynetim::IndependentCascadeModel::get_activated_nodes,
...
Returns:
    Set[int]: 激活节点集合。仅在 record_activated 为 True 时有效。
            ...
            .def("get_activation_frequency",
                &pynetim::IndependentCascadeModel::get_activation_frequency,
...
Returns:
    List[int]: 激活频数列表，索引 i 表示节点 i 被激活的次数。
            ...
            .def("run_monte_carlo_diffusion",
                [](pynetim::IndependentCascadeModel& self, int mc_rounds, py::object random_seed_obj, bool use_multithread = false, int num_threads = 0, bool normalize = false) {
