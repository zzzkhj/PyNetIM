...
Args:
    record: 是否记录激活频数。
            ...
            .def("run_single_simulation",
                [](pynetim::IndependentCascadeModel& self, py::object seed_obj) {
...
Returns:
    List[int]: 激活频数列表，索引 i 表示节点 i 被激活的次数。
            ...
            .def("run_monte_carlo_diffusion",
                [](pynetim::IndependentCascadeModel& self, int rounds, py::object seed_obj, bool use_multithread = false, int num_threads = 0) {
                    bool use_random_seed = seed_obj.is_none();
                    unsigned int seed = use_random_seed ? 0 : py::cast<unsigned int>(seed_obj);
