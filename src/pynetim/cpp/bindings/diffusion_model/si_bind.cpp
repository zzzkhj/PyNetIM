...
Args:
    record: 是否记录感染频数。
            ...
            .def("run_single_simulation",
                [](pynetim::SusceptibleInfectedModel& self, py::object seed_obj) {
...
Returns:
    List[int]: 感染频数列表，索引 i 表示节点 i 被感染的次数。
            ...
            .def("run_monte_carlo_diffusion",
                [](pynetim::SusceptibleInfectedModel& self, int rounds, py::object seed_obj, bool use_multithread = false, int num_threads = 0) {
                    bool use_random_seed = seed_obj.is_none();
                    unsigned int seed = use_random_seed ? 0 : py::cast<unsigned int>(seed_obj);
