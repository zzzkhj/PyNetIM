...
PYBIND11_MODULE(opim_algorithm, m) {
    ...
    py::class_<pynetim::OPIMAlgorithm, std::shared_ptr<pynetim::OPIMAlgorithm>>(m, "OPIMAlgorithm")
...
Args:
    ...
    verbose: 是否显示关键过程日志，默认为 False。
        ...
        .def("run", &pynetim::OPIMAlgorithm::run,
            ...
            py::arg("delta") = -1.0,
            py::arg("mode") = 2,
            R"doc(run(k: int, num_rr_sets: int, delta: float = -1.0, mode: int = 2) -> set[int]

执行OPIM算法选择种子节点。

OPIM使用固定数量的RR集合，最大化近似保证。

Args:
    k: 需要选择的种子节点数量。
    num_rr_sets: RR集合总采样数量（会均分给R1和R2）。
    delta: 失败概率参数，默认为 1/n。
    mode: 上界计算模式：
        - 0: Vanilla版本，返回(1-1/e)-近似
        - 1: 使用最后一轮的上界
        - 2: 使用所有轮中的最小上界（默认）
...
Args:
    ...
    verbose: 是否显示关键过程日志，默认为 False。
        ...
        .def("run", [](pynetim::OPIMCAlgorithm& self, int k, double epsilon, double delta, int mode) {
                return self.run(k, epsilon, delta, mode);
            },
            py::arg("k"),
            py::arg("epsilon"),
            py::arg("delta") = -1.0,
            py::arg("mode") = 2,
            R"doc(run(k: int, epsilon: float, delta: float = -1.0, mode: int = 2) -> set[int]
